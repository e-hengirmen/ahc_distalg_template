...
.. code-block:: RST
    ...
    Implements: 
    Uses: send_message  # sends message to given node
    Events: Init, MessageNeighboor, OnMessageFromNeighboor
    Needs: adjacent_nodes_set: ans

    OnInit: () do
        remaining_ans = ans.deep_copy()
        remaining_neighboor_count = remaining_ans.length()
        If remaining_neighboor_count == 1:
            is_leaf = True
        Else:
            is_leaf = False
    
    MessageNeighboor: ( x ) do
        send_message(x)  # you are my father Vader
    
    OnMessageFromNeighboor: ( x ) do
            If x in remaining_ans:
            remaining_ans.
...
received a parent request on all ports or on every port except one. Since leaf nodes have at
...
sends a parent request on the remaining port (if any) on which it has not received a parent
request. In figure 2, two links have been assigned as “child links” and one parent request
from a leaf node is still pending. In the third stage, if a node has received parent requests
on all ports but has not sent a parent request itself, then it decides that it is the root of the
